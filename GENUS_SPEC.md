...
2. **Confidence honesty.** Every output carries an honest signal of its uncertainty. Silent fakery erodes trust faster than any failure mode.

   For agents emitting claims to Genus surfaces (substrate stores, dashboards, module views, operator-facing artifacts), the structured form is **ConfidenceFrame v1** — three independent layers (Mental Model / Data Grounding / Coherence), aggregated by `min`, with an L1 hard gate. Per-claim, not per-module. See [CONFIDENCE_FRAME.md](./CONFIDENCE_FRAME.md) for the full spec, wire format, policy modes (`silent` / `warn` / `block`), and conformance test. Bound agents declare their per-surface mode via `confidence_frame_mode` in the manifest.

   The legacy tag-only convention (✅ high / ⚠️ partial / ❓ speculative) is **grandfathered for agents that do not emit claims to Genus surfaces** — e.g. raw chat output, conversational replies that produce no substrate writes or dashboard claims. Such agents satisfy #2 with the tag-only form.
