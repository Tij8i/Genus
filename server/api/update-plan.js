...
export async function onRequestPost({ request, env }) {
  ...
  if (action === 'complete_cycle') {
    ...
    // Release child initiatives back to the backlog. They were vetted enough
    // to be in a plan; default them to 'ready'. Operator can further triage
    // or discard individually from the Backlog.
      ...
      try {
        ...
        if (Array.isArray(initiatives)) {
          ...
          for (const iid of plan.initiative_ids) {
            const init = initiatives.find(i => i.id === iid);
            if (!init) continue;
            // Only release if it was tied to THIS plan (defence against
            // the same initiative being reassigned to another active plan).
        ...
        // Non-fatal — plan is already marked discarded; log for the response.
