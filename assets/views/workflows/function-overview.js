...
//
// Attention banner + 4 stat tiles + 2 rollup cards (Workflows preview +
// Open tasks preview) + Information tiles (module-specific).
...
export async function renderFunctionOverview(mod) {
  ...
  const banner = renderAttentionBanner(nowTasks.length, overdueTasks.length, mod);
  ...
  root.innerHTML = `
    <div style="max-width:1080px;margin:0 auto;padding:22px 28px 80px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:12px;">
        ${(INFO_TILES[mod] || []).map(infoTile).join('')}
      </div>
    </div>
  `;
  document.getElementById('add-workflow-btn')?.addEventListener('click', () => {
    alert('+ Add workflow — overlay ships in the follow-up slice (per the brief: presentational in prototype, wire to real create).');
  });
}
