...
Prior reading: `GENUS_SPEC.md` (the protocol), `AGENT_FAMILIES.md` (the three families), and the relevant archetype spec (`VIRGIL.md` / `STEWART.md` / `MASON.md`).
...
| `archetype` | enum | yes | One of `"virgil"`, `"stewart"`, `"mason"`. The family this agent belongs to. |
...
| `archetype` | Must be exactly one of `"virgil"`, `"stewart"`, `"mason"`. No others permitted in v0.1. |
...
- [ ] Honors archetype constraints (Mason `heartbeat_schedule: null`, Stewart `business_unit` singular, Virgil `personal_data_scope` declared, etc.)
...
- Want to understand the three archetypes? → `AGENT_FAMILIES.md`
