...
export async function onRequestPost({ request, env }) {
  ...
  // below — no operator "Finalize" click, no draft-limbo state.
  ...
  // Write all 4 files (sequential — shas chain), then auto-file finalize task.
  ...
  try {
    ...
    try {
      finalizeTaskId = await autoFileFinalizeTask({
        pat: env.GITHUB_PAT, bu, plan: newPlan,
        planGoals: newGoalIds.map(id => goals.find(g => g.id === id)).filter(Boolean),
