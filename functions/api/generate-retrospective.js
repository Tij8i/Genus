...
// for a completed plan. See server/api/generate-retrospective.js for the
// canonical implementation + scheduler-facing export.
//
// Body: { bu, plan_id }
// Returns: { ok, task, stewart, tasks_commit_sha, plans_commit_sha }
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!Array.isArray(plans)) return jsonResponse(500, { ok: false, message: 'plans.json not an array' });
    const plan = plans.find(p => p.id === planId);
    if (!plan) return jsonResponse(404, { ok: false, message: `Plan ${planId} not found` });
    if (plan.status !== 'completed') return jsonResponse(409, { ok: false, message: `Retrospective only valid on completed plans (${planId} is ${plan.status})` });
    if (plan.retrospective_generated_at) return jsonResponse(409, { ok: false, message: `Retrospective already generated at ${plan.retrospective_generated_at}` });

    const stewart = await resolveStrategyStewart(env.GITHUB_PAT, bu);
    if (!stewart) return jsonResponse(400, { ok: false, message: `No Strategy Stewart bound to BU '${bu}'. Install the strategy module first.` });

    let initiatives = [];
    let kpis = [];
    try { initiatives = JSON.parse((await getFile(env.GITHUB_PAT, `dashboard/public/data/bus/${bu}/initiatives.json`)).content); } catch {}
    try { kpis = JSON.parse((await getFile(env.GITHUB_PAT, `dashboard/public/data/bus/${bu}/kpis.json`)).content); } catch {}
    ...
    const tasksFile = await getFile(env.GITHUB_PAT, tasksPath);
    const tasks = JSON.parse(tasksFile.content);
    if (!Array.isArray(tasks)) return jsonResponse(500, { ok: false, message: 'tasks.json not an array' });
    ...
    const newTask = {
      id: taskId, bu,
      title: `Retrospective for ${planId} (30d post-close)`,
      description,
      origin: 'auto_retro',
      proposer: 'scheduler',
      proposed_at: now,
      ...
      risk_level: 'low', reversibility: 'high', estimated_minutes: 20,
      target: { type: 'retrospective_synthesis', scope: `dashboard/public/data/bus/${bu}/retrospectives.jsonl`, executor: stewart.agent_id },
      advances_initiative: null, advances_plan: planId, affects_kpi: null, from_memo: null,
      requires_operator_input: false, operator_input_prompt: null,
      status: 'approved',
      approval: { rule_evaluation: 'auto_retro → auto-approved (scheduled at plan.completed_at + 30d)', decided_by: 'scheduler', decided_at: now, notes: null },
      execution: { paperclip_issue_id: null, paperclip_issue_url: null, started_at: null, completed_at: null, outcome: null },
    };
    tasks.push(newTask);
    const tasksCommit = await putFile(env.GITHUB_PAT, tasksPath, JSON.stringify(tasks, null, 2) + '\n', tasksFile.sha,
      `tasks: ${taskId} — auto-retro for ${planId} (→ ${stewart.agent_id})`);
    ...
    const p2 = plans2.find(p => p.id === planId);
    if (p2) { p2.retrospective_generated_at = now; p2.retrospective_task_id = taskId; }
    const plansCommit = await putFile(env.GITHUB_PAT, plansPath, JSON.stringify(plans2, null, 2) + '\n', plansFile2.sha,
      `plans: ${planId} — retrospective task ${taskId} filed`);

    return jsonResponse(200, {
      ok: true, task: newTask, stewart: stewart.agent_id,
      tasks_commit_sha: tasksCommit.commit.sha, plans_commit_sha: plansCommit.commit.sha,
      message: `Retrospective task ${taskId} filed for plan ${planId} (→ ${stewart.agent_id}).`,
    });
  } catch (e) {
